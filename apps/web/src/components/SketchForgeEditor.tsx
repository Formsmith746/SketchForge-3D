...
import { recordSuccessfulCreator, reportEditorHealthFailure, trackTelemetryOncePerSession } from "@/lib/telemetry";
...
import { EditorTutorialOverlay, TutorialCompletionWindow, type EditorTutorialStep } from "./tutorial/EditorTutorial";
import {
  canonicalizeShape,
  cleanNearZero,
...
type TopPanel = "import" | "export" | "tips" | "settings" | null;
...
function getManifoldRuntime() {
  ...
  manifoldRuntimePromise ??= runtimeModule
    ...
    .catch((error) => {
      ...
      if (typeof window !== "undefined") {
        reportEditorHealthFailure("wasm-initialization-failed");
      }
      throw error;
    });
...
const DEMO_TUTORIAL_FINISHED_KEY = "sketchforge.demoTutorial.finished.v1";
...
export function SketchForgeEditor({
  ...
  historySettingsEnabled?: boolean;
} = {}) {
  ...
  const [tutorialStep, setTutorialStep] = useState<EditorTutorialStep>("complete");
  const [tutorialCompletionOpen, setTutorialCompletionOpen] = useState(false);
  ...
  useEffect(() => {
    if (!tutorialEnabled) {
      ...
      setTutorialCompletionOpen(false);
      return;
    }

    let tutorialFinished = false;
    try {
      tutorialFinished = window.localStorage.getItem(DEMO_TUTORIAL_FINISHED_KEY) === "true";
    } catch {
      // The tutorial still works when browser privacy settings block local storage.
    }
    setTutorialStep(tutorialFinished ? "complete" : "welcome");
    if (!tutorialFinished) trackTelemetryOncePerSession("tutorial-started");
  }, [tutorialEnabled]);
  ...
  const commitShapes = useCallback(
    (next: WorkplaneShape[], nextSelection: string | string[] | null = selectedIds, message?: string) => {
      ...
      if (changed) {
        if (canonicalNext.length > 0) recordSuccessfulCreator();
  ...
  useEffect(() => {
    if (!localAutomationEnabled || process.env.NODE_ENV === "production" || typeof window === "undefined") {
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (tutorialStep === "welcome" || tutorialStep === "workspace" || tutorialStep === "inspector") {
        return;
      }
  ...
  const tutorialModalOpen = tutorialStep === "welcome" || tutorialStep === "workspace";
  const tutorialInteractionLocked = tutorialModalOpen || tutorialStep === "inspector";
  const finishTutorial = (showCompletion: boolean) => {
    trackTelemetryOncePerSession("tutorial-started");
    trackTelemetryOncePerSession(showCompletion ? "tutorial-completed" : "tutorial-skipped");
    try {
      window.localStorage.setItem(DEMO_TUTORIAL_FINISHED_KEY, "true");
    } catch {
      // Completion remains active for this session if local storage is unavailable.
    }
    setTutorialStep("complete");
    setTutorialCompletionOpen(showCompletion);
  };
  const advanceTutorial = () => {
    trackTelemetryOncePerSession("tutorial-started");
    setTutorialStep((current) => {
      if (current === "welcome") return "workspace";
      if (current === "workspace") return "shapes";
      if (current === "inspector") return "resize";
      return current;
    });
  };

  return (
    <div className={`sketchforge-editor ${tutorialModalOpen ? "tutorial-modal-open" : ""}`}>
      <SecondaryToolbar
        ...
        onTutorialShapeMenuOpened={() => setTutorialStep((current) => current === "shapes" ? "cube" : current)}
        onTutorialCubeAdded={() => setTutorialStep((current) => current === "cube" ? "inspector" : current)}
        onTopPanel={(panel) => {
      ...
      <div className="editor-body">
        {toolbarMode === "sketch" && sketchActive ? (
          ...
          interactionLocked={tutorialInteractionLocked}
          tutorialResizeActive={tutorialStep === "resize"}
          onTutorialResizeStart={() => finishTutorial(true)}
          themePreference={themePreference}
          resolvedTheme={resolvedTheme}
          onThemePreferenceChange={onThemePreferenceChange}
          historySettingsEnabled={historySettingsEnabled}
          />
        )}
      </div>
      {edgeModifier ? (
      ...
      />
      {tutorialCompletionOpen ? (
        <TutorialCompletionWindow onClose={() => setTutorialCompletionOpen(false)} />
      ) : null}
...
function SecondaryToolbar({
  ...
  tutorialStep: EditorTutorialStep;
  onTutorialShapeMenuOpened: () => void;
  onTutorialCubeAdded: () => void;
  ...
  const addShapeFromMenu = (shape: ShapeAsset) => {
    onAddShape(shape);
    if (shape.id === "box") onTutorialCubeAdded();
    setShapesOpen(false);
  };
          ...
          <div className="toolbar-section-tools">
            <button
              ...
              onClick={() => {
                ...
                setShapesOpen(next);
                if (next) onTutorialShapeMenuOpened();
              }}
            >
              <ToolbarShapeAddIcon />
            </button>
            {tutorialStep === "shapes" ? (
              <button
                className="tutorial-control-arrow tutorial-shapes-arrow"
                type="button"
              ...
              >
                <span>Open Shapes</span>
                <svg viewBox="0 0 44 52"><path d="M22 49V8m0 0L7 23M22 8l15 15" /></svg>
              </button>
            ) : null}
          </div>
          {shapesOpen ? (
            <div className={`shape-menu-dropdown ${tutorialStep === "cube" ? "tutorial-cube-open" : ""}`}>
              ...
              {tutorialStep === "cube" ? (
                <div className="tutorial-control-arrow tutorial-cube-arrow" aria-hidden="true">
                  <svg viewBox="0 0 54 42"><path d="M50 21H7m0 0l15-15M7 21l15 15" /></svg>
                  <span>Choose the cube</span>
                </div>
              ) : null}
            </div>
          ) : null}
